...
cmd = (
    ...
    f"--max-model-len 2048 --gpu-memory-utilization 0.85 --api-server-count 1"
)
log = open(f"/results/sse/server_{LABEL}.log", "w")
...
if PIN:
    # frontend -> 1 core; engine -> its own cores; client gets the rest.
    subprocess.run(["taskset", "-acp", "0", str(fe_pid)], capture_output=True)
    if eng_pid:
        subprocess.run(["taskset", "-acp", "1-8", str(eng_pid)], capture_output=True)
...
if res["duration_s"]:
    # cores consumed by the frontend during the request phase; ~1.0 means the
    # pinned frontend is saturated and is therefore the bottleneck.
    res["frontend_cores"] = round((c1 - c0) / res["duration_s"], 3)

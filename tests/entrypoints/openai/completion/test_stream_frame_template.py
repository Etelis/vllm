...
def test_delta_chunk_fields_are_frozen():
    """The template is only equivalent to the model path while a plain delta
    chunk populates exactly these fields. Deriving the template tracks field
    order and formatting automatically, but a NEW field that the generator
    starts populating per chunk would be dropped silently -- so adding one
    must fail here and force the fast path to be revisited."""
    ...
    assert set(CompletionStreamResponse.model_fields) == {
        "id",
        "object",
        "created",
        "model",
...
def test_serialization_failure_is_not_raised_by_the_template():
    """A lone surrogate in a client-supplied request id fails to serialize on
    both paths; the template must not turn that into an exception raised
    before streaming starts."""
    with pytest.raises(PydanticSerializationError):
        build_delta_frame_template("cmpl-\ud800", CREATED, "m")
    # ...and the model path fails the same way, so falling back to it in
    # completion_stream_generator preserves the existing error behaviour.
    with pytest.raises(PydanticSerializationError):
        model_frame("cmpl-\ud800", CREATED, "m", 0, "hi")
